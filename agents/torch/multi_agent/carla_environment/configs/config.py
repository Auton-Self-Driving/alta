...
from carla_environment.configs import (
    model_configs,
    observation_configs, 
    action_configs, 
    scenario_configs, 
...
print(model_configs.BaseModelConfig())
...
class BaseMainConfig(BaseConfig):
    ...
    def __init__(self):
        ...
        # Number of agents to train per worker
        self.num_agents = None

        # Devices to run workers
    ...
    def populate_config(self,
        ...
        if(isinstance(model_config, str)):
            ...
            config_type = getattr(model_configs, model_config)
    
            # Instantiate Object
            self.model_config = config_type()
        elif(isinstance(model_config, model_configs.BaseModelConfig)):
            # Just save object, since it is already instantiated
            self.model_config = model_config
        else:
            # Invalid Argument
            raise Exception("Invalid argument for model_config")
        ...
        # Carla GPU
        self.carla_gpu = 'cuda:{}'.format(carla_gpu)

        # Worker GPUs
        self.device_list = ['cuda:{}'.format(dev) for dev in device_list]
...
class DefaultTestConfig(DefaultMainConfig):
