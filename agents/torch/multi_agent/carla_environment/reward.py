...
def compute_reward(prev, current, config, verbose=False):
    ...
    # Static Reward
    static_reward = 0
    if current['static_steps'] > config.scenario_config.max_static_steps:
        static_reward = -1 * reward_config.static_penalty
    ...
    # Unclipped and unnormalized reward for logging
    raw_reward = reward

    # Clip reward
    if reward_config.clip_reward:
    ...
    if verbose:
        print("dist_to_trajectory_reward, speed_reward, acceleration_reward, collision_reward, light_reward, steer_reward, static_reward, success_reward, raw_reward, reward")
        print(dist_to_trajectory_reward, speed_reward, acceleration_reward, collision_reward, light_reward, steer_reward, static_reward, success_reward, raw_reward, reward)

    # This is computed in env._get_ego_input
