...
def vis_policyloss(vis, model_dir_path, replay_buffer, fname=None, tseries = False, gpu='3', st = 0, ed= -1):
	...
	env = CarlaEnv(config=config.config, vis_wrapper=None, logger=None, log_dir = ALTA_LOGS)
	...
	if tseries:
		...
		vis.line(X = np.arange(len(q1_values[st:ed])), Y = q1_values[st:ed], opts = {'title': 'q1Values_'+str(filename), 'width':1200, 'height':600})
...
def vis_qvalues2(vis, model_dir_path, replay_buffer, fname=None, tseries = False, gpu='3', st = 0, ed= -1):
    ...
    env = CarlaEnv(config=config.config, vis_wrapper=None, logger=None, log_dir = os.path.join(model_dir_path, ALTA_LOGS+'_runid_run'+str(1)))
    dummy_env = DummyVecEnv([lambda: env])
    ...
    for filename in filenames:
        ...
        for run_id in run_ids:
            ...
            q1_values = np.zeros((len(replay_buffer), ))
            q1_values_pred = np.zeros((len(replay_buffer), ))
            stp1_values = np.zeros((len(replay_buffer), ))
            std_values = np.zeros((len(replay_buffer), 2))
            pred_actions = np.zeros((len(replay_buffer), 2))
            gt_actios = np.zeros((len(replay_buffer), 2))
            q1_loss = np.zeros((len(replay_buffer), ))
            q2_loss = np.zeros((len(replay_buffer), ))
            ...
            for i in range(len(done_idxs))[:-1]:
                ...
                _, batch_std_values = model.predict_proba_step(batch_obs)
                ...
                out = model.sess.run(model.step_ops, feed_dict)
                policy_loss_pred, qf1_loss_pred, qf2_loss_pred, value_loss_pred, qf1_pred, qf2_pred, *values = out
				##################################### Compare #####################################
                ...
                q1_values_pred[done_idxs[i]+1:done_idxs[i+1]+1] = np.squeeze(qf1_pred)				
                # stp1_values[done_idxs[i]+1:done_idxs[i+1]+1] = np.squeeze(next_s_val)
                std_values[done_idxs[i]+1:done_idxs[i+1]+1,:] = np.squeeze(batch_std_values)
                pred_actions[done_idxs[i]+1:done_idxs[i+1]+1,:] = np.squeeze(batch_pred_actions)
                gt_actios[done_idxs[i]+1:done_idxs[i+1]+1,:] = np.squeeze(batch_actions)                
                #out = model._train_step(step, None, 0.0)
        ...
        if tseries:
            q1_residual = q1_values-q1_values_pred
            hold = np.where(q1_residual[:100000]<0)[0]
            print_logs.append(filename+"-->"+str(len(hold)*100.0/100000))
            #vis.line(X = np.arange(len(q1_residual[st:ed])), Y = q1_residual[st:ed], opts = {'title': 'q1_residual_'+str(filename), 'width':1200, 'height':600})
        else:
            #s = 0
            #e = -1
            #vis.line(X = np.arange(len(q1_loss[st:ed])), Y = q1_loss[st:ed], opts = {'title': 'q1Loss', 'width':1200, 'height':600})
            vis.line(X = np.arange(len(q1_values[st:ed])), Y = q1_values[st:ed], opts = {'title': 'q1Values_'+str(filename), 'width':1200, 'height':600})
            # vis.line(X = np.arange(len(pred_actions[st:ed,0])), Y = pred_actions[st:ed,0], opts = {'title': 'steer_pred', 'width':1200, 'height':600})
            # vis.line(X = np.arange(len(gt_actios[st:ed,0])), Y = gt_actios[st:ed,0], opts = {'title': 'steer_gt', 'width':1200, 'height':600})        
            # vis.line(X = np.arange(len(pred_actions[st:ed,1])), Y = pred_actions[st:ed,1], opts = {'title': 'speed_pred', 'width':1200, 'height':600})
            # vis.line(X = np.arange(len(gt_actios[st:ed,1])), Y = gt_actios[st:ed,1], opts = {'title': 'speed_gt', 'width':1200, 'height':600})        
...
if __name__=='__main__':
	...
	if args.qvalues:
		...
		if args.tseries:
			...
			filename = 'sac_weights150000.pkl'
			if not args.model_name=='':
				filename = args.model_name
			vis_qvalues2(vis, args.exp_path, buff, filename, tseries=True, gpu=args.gpu)
