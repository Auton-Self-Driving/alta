...
#          label_names = ['Navigation task']
    ...
    for ind in range(len(timesteps)):
        ...
        max_success_i = max_success[ind]
        plt.plot(tsteps[:-5], mean_success_i[:-5] , label=label_names[ind], color=mean_colors[ind])
        if with_std:
            plt.fill_between(tsteps[:-5], min_success_i[:-5], max_success_i[:-5], color=fill_colors[ind], alpha=alphas[ind])
...
if __name__ == "__main__":
    ...
    files = list(filter(os.path.isfile, glob.glob(log_path + "*")))
