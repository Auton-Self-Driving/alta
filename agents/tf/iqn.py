...
class C51_Agent:
    def __init__(self, 
        ...
        self.quantile_embedding_dim = embedding_dim #128
        self.f_arch = f_arch
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            ...
            state_net = tf.layers.dense(inputs=state_reshape, units=self.quantile_embedding_dim, activation=tf.nn.relu)#tf.nn.selu)
            ...
            for layer in self.f_arch: 
                net = tf.layers.dense(inputs=net, units=layer, activation=tf.nn.relu)
    ...
    def choose_action(self, state):
        ...
        # t = np.random.rand(self.upper, self.num_support)
