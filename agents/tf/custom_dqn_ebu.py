...
from custom_build_graph_ebu import custom_build_train_ebu
...
def get_save_best_model(total_rewards, total_successes, model_file_names, path):
    ...
    best_model = Custom_DQN_EBU.load(best_file_name)
...
class Custom_DQN_EBU(DQN):
    '''
    Custom_DQN_EBU class with addition of testing within training in DQN class.
    '''
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=5e-4, buffer_size=50000, exploration_fraction=0.1,
                 ...
                 _init_setup_model=True, policy_kwargs=None, full_tensorboard_log=False, seed=None, ebu_beta=0.5):

        self.ebu_beta = ebu_beta
        ...
        super(Custom_DQN_EBU, self).__init__(policy=policy, env=env, gamma=gamma, learning_rate=learning_rate, buffer_size=buffer_size,
    ...
    def setup_model(self):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                self.act, self._train_step, self.update_target, self.step_model, self.target_policy = custom_build_train_ebu(
    ...
    def learn_new_EBU(self, env, total_timesteps, callback=None, log_interval=100, tb_log_name="DQN",
                reset_num_timesteps=True, replay_wrapper=None, save_file="dqn_weights", num_opt_epochs=5):

            '''
            This is learn method for Episodic Backward Update (EBU).
            This is built on learn_new method which includes saving additional information of 
            termination_state_code, time_to_termination for each transition in replay buffer.
            '''
            ...
            with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
                ...
                for _ in range(self.num_timesteps, total_timesteps):
                    ...
                    # Also checking that there are more than 10 episodes in replay buffer
                    can_sample = self.replay_buffer.can_sample(self.batch_size) \
                            and len(self.replay_buffer._done_idx) > 10
                    if can_sample and self.num_timesteps > self.learning_starts \
                        ...
                        for i_opt in range(num_opt_epochs):
                            ...
                            summary, td_errors, run_metadata = self._do_training(writer)
    ...
    def _do_training(self, writer):

        '''
        Training method written using EBU source code
        '''
        ...
        if self.batchnum == self.batch_count:
            ...
            batchnum, sample = self.replay_buffer.sample_random_episode(self.batch_size)
            obses_t, actions, rewards, obses_tp1, dones, infos, _ = sample
            self.epi_state, self.epi_actions, self.epi_rewards, self.epi_terminals = obses_t, actions, rewards, dones
            self.batchnum = batchnum
            self.epi_len = self.batchnum * self.batch_size

            for i in range(self.batchnum):
                self.Q_tilde = np.append(self.Q_tilde, self.get_target_q_values(self.epi_state[self.batch_size * i:self.batch_size * (i+1)]))
            
            self.Q_tilde = np.reshape(self.Q_tilde, (self.batchnum * self.batch_size, self.action_space.n))
            self.Q_tilde = np.roll(self.Q_tilde, self.action_space.n) # Q(S2) becomes the first column
            
            for i in range(self.epi_len):
                if self.epi_terminals[i]:
                    self.Q_tilde[i,:] = [0]*self.action_space.n
            ...
            for i in range(0, self.epi_len):
                if i < self.epi_len - 1:
                    ...
                    if self.epi_terminals[i]:
                        ...
                        self.Q_tilde[i+1,self.epi_actions[i]] = self.ebu_beta * self.y_ [i] + (1-self.ebu_beta)*self.Q_tilde[i+1,self.epi_actions[i]]
            ...
            obses = self.epi_state[0:self.batch_size]
            actions = np.reshape(self.epi_actions[0:self.batch_size],(self.batch_size,))
            targets = np.reshape(self.y_[0:self.batch_size],(self.batch_size,))
            rewards = np.reshape(self.epi_rewards[0:self.batch_size],(self.batch_size,))
        ...
        # if an episode is still being updated, use the next minibatch of the already generated target value.
            ...
            obses = self.epi_state[(self.batch_count-1)*self.batch_size:self.batch_count*self.batch_size]
            actions = np.reshape(self.epi_actions[(self.batch_count-1)*self.batch_size:self.batch_count*self.batch_size],(self.batch_size,))
            targets = np.reshape(self.y_[(self.batch_count-1)*self.batch_size:self.batch_count*self.batch_size],(self.batch_size,))
            rewards = np.reshape(self.epi_rewards[(self.batch_count-1)*self.batch_size:self.batch_count*self.batch_size],(self.batch_size,))
            weights = np.ones_like(rewards)
        
        # Logging logic for training
        ...
        if writer is not None:
            ...
            if (1 + self.num_timesteps) % 10000 == 0:
                ...
                summary, td_errors = self._train_step(obses, actions, targets, rewards, weights, sess=self.sess,
            ...
            _, td_errors = self._train_step(obses, actions, targets, rewards, weights, sess=self.sess)

        return summary, td_errors, run_metadata
    ...
    def get_target_q_values(self, observation, state=None, mask=None, deterministic=True):
        observation = np.array(observation)
        vectorized_env = self._is_vectorized_observation(observation, self.observation_space)

        observation = observation.reshape((-1,) + self.observation_space.shape)
        with self.sess.as_default():
            actions, q_values, actions_proba = self.target_policy.step(observation, deterministic=deterministic)
