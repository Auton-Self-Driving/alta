...
if __name__ == '__main__':
    ...
    #             model = MY_SAC.load(SAVE_PATH, dummy_env)
    #             success_episodes = 0
    #             results = {}
    ...
    #             model = MY_SAC(policy=MlpPolicy, env=env, learning_rate=3e-4,
    #              tensorboard_log=TB_LOGS_DIR, full_tensorboard_log=True, verbose=1)
    ...
    #                 model = MY_SAC.load(latest_model, dummy_env)
    #                 print("Model: {} loaded successfully".format(latest_model))
    #                 env.total_steps = completed_steps
    #                 env.episode_num = completed_episodes
    #                 _ = model.learn(steps, completed_steps, env, tb_log_name="SAC", save_file=SAVE_PATH, reset_num_timesteps=False, seed=seed)    
    #             else:
    #                 dt = datetime.now()
    ...
    #                     f.write(str(millis))
    #                 random_model = model.learn(0, 0, env, tb_log_name="SAC", save_file=SAVE_PATH, reset_num_timesteps=True, seed=millis)
    #                 random_model.save(SAVE_PATH + "0")
    #                 plot_policy_and_value_fns(random_model, 0, POLICY_PLOTS)
    #                 _ = model.learn(steps, 0, env, tb_log_name="SAC", save_file=SAVE_PATH, reset_num_timesteps=True, seed=millis)
