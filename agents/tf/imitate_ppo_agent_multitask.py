    ...
    for ind in range(env.config["num_episodes"]):
        ...
        while not done:
            ...
            if rv:
                ...
                preds = model.predict(rv_img, manual_states, ret_pred_states=True)
                actions = preds[0][0]
                pred_states = preds[1][0]
                pred_states[0] = 1.0/(1.0+np.exp(-1.0*pred_states[0]))
                pred_states[-1] = 1.0/(1.0+np.exp(-1.0*pred_states[-1]))
            else:
                img = np.expand_dims(preproc_img(img, image_size), axis = 0)
                actions = model.predict(img, manual_states)[0]
            info = env.step(actions, pred_states)
            reward += info[1][0][0]
            done = info[2]
            obs = info[0]
            rv_img = info[-1]
...
def collect_data(model, env, dump_results=False, path='.', num_ep=10, model_step=None, save = False):
    ...
    for ind in range(num_ep):
        ...
        while not done:
            ...
            if z_dim == 3:
                manual_states = np.hstack([obs[:, :1], obs[:,3:5]])
                helper_states = np.hstack([obs[:, 1], obs[:,-1]])
            elif z_dim == 5:
                manual_states = np.hstack([obs[:, :1], obs[:,3:5], obs[:,6:]])
                helper_states = np.hstack([obs[:, 1], obs[:,-1]])
            ...
            to_append = tuple([img, rv_img, manual_states, actions, helper_states])
...
def collect_data_agent(expert_model, agent_model, env, dump_results=False, path='.', num_ep=10, model_step=None, save = False, iter=0):
    ...
    for ind in range(num_ep):
        ...
        while not done:
            ...
            to_append = tuple([img, rv_img, manual_states, expert_actions, helper_states])
    ...
    try:
        ...
        if args.dataset_path is None:
            ...
            with open(ALTA_LOGS + 'test_results_' + config.config["city_name"] +  config.config['scenarios'] +  '_run_' + str(test_idx) + ".txt", "a") as f:
                if args.test:
                    print("Testing")
                    Imitator = AuxNetController(z_size = z_dim, image_size = image_size, buffer_size = 1, gt_size = 2, epoch_per_optimization=50, multi_task='state_pred')
                    Imitator.load(DATA_DIR+'front_dagger_rgbres224_clsfn_sigmoid_iter_3.json')
            ...
            data = pickle.load(open(DATA_DIR+'imitation_data_front_rgbres224.p', 'rb'))
        ...
        Imitator = AuxNetController(z_size = z_dim, image_size = image_size, frame_stack = 1, buffer_size = 4*len(data), gt_size = 2, epoch_per_optimization=50, multi_task='state_pred')
        #Imitator.load(DATA_DIR+'front_dagger_rgbres224_iter_0.json')
        Imitator.buffer = []
        for iter in range(5):
            print("Preprocessing data")
            for data_idx in range(len(data)):
                img = preproc_img(data[data_idx][1], rv_img_sz)
                manual_state = data[data_idx][-3]
                gt = data[data_idx][-2]
                helper_states = 1.0-data[data_idx][-1]
                helper_states[0] = (helper_states[0]>0.5)*1
                helper_states[-1] = (helper_states[-1]>0.2)*1
                Imitator.buffer.append(tuple([img, manual_state, gt, helper_states]))

            Imitator.save_every_epoch = True
            Imitator.model_filepath = DATA_DIR+'front_dagger_rgbres224_clsfn_sigmoid_fl_iter_'+str(iter)+'.json'
            ...
            model = PPO.load(args.agent_model_path, env=dummy_env)
            new_data = collect_data_agent(model, Imitator, env, num_ep = 10, save=False, iter=iter)
            env.reset()
