...
def compute_reward(name, prev_measurement, cur_measurement, config=None, verbose=False):
    if name == 'corl':
        ...
        reward = _compute_reward_SAC(prev_measurement, cur_measurement, config, verbose=verbose)
    return reward
...
def _compute_reward_simple2(prev, current, config=None, verbose=False):
    ...
    reward = dist_to_trajectory_reward + speed_reward + steer_reward + collision_reward
...
def _compute_reward_SAC(prev, current, config=None, verbose=False):
    cur_dist = current["distance_to_goal"]
    prev_dist = prev["distance_to_goal"]
    dist_to_goal_reward = -0.01 * current['dist_to_trajectory']
    current["dist_to_goal_reward"] = dist_to_goal_reward
    ...
    reward = dist_to_goal_reward + speed_reward + steer_reward + collision_reward
