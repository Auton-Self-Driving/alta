...
#from agents.torch.resnet_lbc.rgb_resnet import get_resnet
import matplotlib
...
class CarlaEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        if self.config["input_type"] == 'vae' or self.config["input_type"] == 'wp_vae':
            ...
            encoded_image = self.resnet_observation(sensor_image[:,:,0])
    ...
    def resnet_observation(self, observation_image):
        model = self.resnet(observation_image)
        model.eval()
        ob = model(observation_image)
        return ob
    ...
    def _reset(self, unseen=False, index=0):
        ...
        if self.config["input_type"] == 'vae' or self.config["input_type"] == 'wp_vae':
            ...
            encoded_image = self.resnet_observation(image[:,:,0])
