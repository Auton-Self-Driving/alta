...
class CarlaEnv(gym.Env):
    def __init__(self, config=DEFAULT_ENV, vis_wrapper=None, vis_wrapper_vae=None, logger=None, log_dir=None, base_prefix='', prefix=''):
        ...
        self.args_longitudinal_dict = {						        	#Controller configurations
            'K_P': 0.1,
            'K_D': 0.0005,
            'K_I': 0.4,
            'dt': 1/10.0}
        self.controller = controller.PIDLongitudinalController(K_P=self.args_longitudinal_dict['K_P'],
        ...
        self.stacked_observation_queue = queue.Queue(maxsize=self.config['frame_stack_size'])	# What is this?

        self.source_transform = None					# Will be assigned in reset function
        self.destination_transform = None				# Will be assigned in reset function
        ...
        # Adding some more episode measurements and initializing
        ...
        if self.config["testing"]:
            ...
            '''if not os.path.exists(os.path.join(self.log_dir, "spawn_pt_order.npy")):
                np.save(os.path.join(self.log_dir, "spawn_pt_order.npy"), spawn_pt_idx)
            else:
                np.save(os.path.join(self.log_dir, "spawn_pt_order_2.npy"), spawn_pt_idx)'''
            np.save(os.path.join(self.log_dir, "spawn_pt_order.npy"), spawn_pt_idx)
        ...
        if self.config["train_config"] == "PPO":
            if self.config["videos"] and self.unseen:
                if self.vis_wrapper is not None:
                    self.vis_wrapper.save_pil_image(convert_to_rgb(reduce_classes(obs['image'][:, :, 0]),\
                                                                    reduced_classes=True).astype(np.uint8),\
                                                    self.num_steps,\
                                                    self.episode_measurements)
                if self.vis_wrapper_vae is not None:
                    decoded_observation = self.vae.decode(encoded_observation)
                    self.vis_wrapper_vae.save_pil_image(convert_to_rgb(convert_from_one_hot(decoded_observation[0, :, :, -5:]),\
                                                                                            reduced_classes=True).astype(np.uint8),\
                                                        self.num_steps,\
                                                        self.episode_measurements)
    ...
    def reset(self, unseen=False, index=0, saved_scenarios = None):
        return self._reset(unseen, index, saved_scenarios)
	...
	#############
        ...
        # Currently scenarios defined only for Town01
        ################################################
        ...
        self.trace_route  = self.global_planner._trace_route(self._map,
                                self.source_transform, self.destination_transform)		# Gets the route
        self.global_planner.set_global_plan(self.trace_route)				        # Fills in the waypoint queue

        ################################################
        # Start vehicle and other actors
        ################################################
        try:
        ...
        if not skip_spawn:
            if self.config["num_npc"] > 0:
               if unseen:
                   number_of_vehicles = self.config["num_npc_lower_threshold"]
               else:
                   number_of_vehicles = np.random.randint(low=self.config["num_npc_lower_threshold"],\
    ...
    def spawn_npc(self, number_of_vehicles, unseen, index=1):
        ...
        if self.config["scenarios"] == "long_straight" and not index:
            ...
            for spawn_point in spawn_points:
                #if unseen:
                #    spawn_flag = self.try_spawn_random_vehicle_at(self.vehicle_blueprints, spawn_point, count)
                #else:
                spawn_flag = self.try_spawn_random_vehicle_at(self.vehicle_blueprints, spawn_point) 
