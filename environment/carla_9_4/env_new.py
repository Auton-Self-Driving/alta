...
import environment.carla_9_4.server_new as server
...
class CarlaEnv(gym.Env):
    def __init__(self, config=DEFAULT_ENV, vis_wrapper=None, vis_wrapper_vae=None, logger=None):
        ...
        # self.camera1_queue = collections.deque(maxlen=10)
        # self.camera2_queue = collections.deque(maxlen=10)
        self.camera1_queue = queue.Queue()
        self.camera2_queue = queue.Queue()
        ...
        self.controller = controller.PIDLongitudinalController(K_P=0.1, K_D=0.0005, K_I=0.4, dt=1/10.0) 
    ...
    def _step(self, action):
        ...
        # hitesh: uncomment following for camera to work
        # cam1_image, cam2_image = self._read_data_new(world_frame, timeout=2.0)
        # dummy_image = np.zeros_like(cam2_image)
        # dummy_image = dummy_image[:, :20]
        # combined_image = np.hstack((cam2_image, dummy_image, cam1_image))
        # print(cam1_image.shape, cam2_image.shape)
    ...
    def _reset(self, unseen=False, index=0):
        ...
        # self.camera1_actor.listen(self.camera1_queue.append)
        # self.camera2_actor.listen(self.camera2_queue.append)
        
        # hitesh
        # self.camera1_actor.listen(self.camera1_queue.put)
        # self.camera2_actor.listen(self.camera2_queue.put)
    ...
    def _read_data_new(self, world_frame, timeout=2.0):

        cam1_image, cam2_image = self.read_camera_data_new(world_frame, timeout)

        # print("raw", cam1_image.shape, cam2_image.shape)

        cam1_image_p, cam2_image_p = self._preprocess_image_new(cam1_image), self._preprocess_image_new(cam2_image)

        print("processed", cam1_image_p.shape, cam2_image_p.shape)

        return cam1_image_p, cam2_image_p

    def _preprocess_image_new(self, image):
    ...
    def read_camera_data_new(self, world_frame, timeout):

        data = [self._retrieve_data(sensor_queue, timeout, world_frame) for sensor_queue in [self.camera1_queue, self.camera2_queue]]
    ...
    def _compute_reward(self, name, prev_measurement, cur_measurement):
        if name == 'corl':
            ...
            reward = self._compute_reward_simple2(prev_measurement, cur_measurement)
        return reward
    ...
    def _compute_reward_simple2(self, prev, current):
        cur_dist = current["distance_to_goal"]
        prev_dist = prev["distance_to_goal"]
        ...
        dist_to_trajectory_reward = -1 * self.dist_to_trajectory
        self.episode_measurements["dist_to_trajectory_reward"] = dist_to_trajectory_reward
        speed_reward = current["speed"]
        acceleration_reward = (current["speed"] - prev["speed"])
        ...
        reward = dist_to_trajectory_reward + speed_reward + steer_reward

        self.episode_measurements["step_reward"] = reward
